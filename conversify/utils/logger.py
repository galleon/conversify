...
def setup_logging(config: dict[str, Any], project_root: str):
    ...
    if not isinstance(level, int):
        print(
            f"Warning: Invalid log level '{log_level_str}' in config. Defaulting to {DEFAULT_LOG_LEVEL}.",
            file=sys.stderr,
        )
        level = logging.INFO
    ...
    if log_file_abs:
        try:
            ...
            print(
                f"Logging configured. Level: {log_level_str.upper()}, File: {log_file_abs}",
                file=sys.stderr,
            )
        except Exception as e:
            print(
                f"Error setting up file logging to {log_file_abs}: {e}", file=sys.stderr
            )
            # Continue with console logging only
    else:
        print(
            f"Logging configured. Level: {log_level_str.upper()}, Console only.",
            file=sys.stderr,
        )
