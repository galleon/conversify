...
def metrics_callback(session: AgentSession):
    ...
    def _on_metrics_collected(ev: MetricsCollectedEvent):
        ...
        if isinstance(agent_metrics, EOUMetrics):
            ...
            logger.info(
                f"TOTAL END TO END LATENCY --> {e2e_latency:.3f}s, EOU: {end_of_utterance_delay:.3f}s, LLM: {llm_ttft:.3f}s, TTS: {tts_ttfb:.3f}s"
            )
    ...
    if agent.memory_handler:
        try:
            ...
            if not hasattr(agent, "chat_ctx") or agent.chat_ctx is None:
                logger.warning(
                    "Agent chat context is not available, skipping memory save."
                )
            else:
