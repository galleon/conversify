...
from livekit.agents import Agent, FunctionTool, llm
...
class ConversifyAgent(Agent):
    ...
    def __init__(
        ...
        self.participant_identity = (
            agent_config["default_participant_identity"] or participant_identity
        )
        ...
        if memory_config["use"]:
            self.memory_handler = AgentMemoryManager(
                participant_identity=self.participant_identity, config=config
            )

        logger.info(
            f"ConversifyAgent initialized for identity: {self.participant_identity}. Memory: {'Enabled' if self.memory_handler else 'Disabled'}"
        )
    ...
    def process_image(self, chat_ctx: llm.ChatContext):
        ...
        should_add_image = any(
            keyword in user_text.lower() for keyword in self.vision_keywords
        )

        if should_add_image:
            logger.info(
                f"Vision keyword found in '{user_text[:50]}...'. Adding image to context."
            )
    ...
    async def tts_node(
        ...
        if cleaned_text_chunks:
            logger.debug(
                f"Sending {len(cleaned_text_chunks)} cleaned chunks to default TTS."
            )
            ...
            async for frame in self.default.tts_node(
                self, text_stream(), model_settings
            ):
                yield frame
