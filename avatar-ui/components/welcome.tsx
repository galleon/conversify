...
export const Welcome = ({
  ...
  const [participantIdentity] = useState(() => `user-${Date.now()}`);

  const handleStartCall = () => {
    // Store participant identity for use in the session
    localStorage.setItem('participant_identity', participantIdentity);
    onStartCall();
  };
      ...
      <div className="w-full max-w-4xl space-y-8 px-4">
        ...
        <div className="flex flex-col items-center space-y-4">
          <div className="flex gap-4">
            <Button
              variant={showFileUpload ? 'outline' : 'secondary'}
              size="lg"
              onClick={() => setShowFileUpload(!showFileUpload)}
              className="w-64 font-mono"
            >
              {showFileUpload ? 'Hide' : 'Upload'} Background Knowledge
            </Button>
          </div>
